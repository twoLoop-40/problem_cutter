...
이미지에서 **문제 번호**를 찾아주세요. 문제 번호는 다음 형태입니다:
- "1.", "2.", "3." (아라비아 숫자 + 점)
- "①", "②", "③" (동그라미 숫자)
- "[1]", "[2]", "[3]" (대괄호 + 숫자)

각 문제 번호에 대해 다음 정보를 JSON 형태로 반환해주세요:
```json
[
  {
    "problem_num": 1,
...
- problem_num: 문제 번호 (숫자만)
- y_coordinate: 이미지 내에서 문제 번호의 Y 좌표 (픽셀, 위에서부터)
- confidence: 확신도 (0.0 ~ 1.0)

**중요:**
- "[1.5점]", "[2점]" 같은 점수 표시는 제외
- 문제 번호만 찾기 (보기 번호 "①", "②" 제외)
- JSON만 반환 (설명 없이)
"""
